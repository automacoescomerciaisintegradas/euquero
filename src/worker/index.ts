...
import { ContactFormSchema, LoginSchema, RegisterSchema, CreditRechargeSchema, type AuthResponse } from "@/shared/types";
...
app.get("/api/auth/github", async (c) => {
  ...
  return c.redirect("/?oauth=github");
});
...
app.post("/api/credits/recharge", zValidator("json", CreditRechargeSchema), async (c) => {
  try {
    const data = c.req.valid("json");
